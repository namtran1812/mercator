...
std::vector<CashFlow> generate_fixed_rate_cashflows(
    ...
    if (annual_coupon_rate < 0.0) {
        throw std::invalid_argument("annual_coupon_rate cannot be negative");
    }
    ...
    if (std::chrono::sys_days{maturity_date} <=
    ...
    const unsigned months_per_period =
        static_cast<unsigned>(12 / payments_per_year);

    const double coupon_amount =
        face_value * annual_coupon_rate /
        static_cast<double>(payments_per_year);

    std::vector<CashFlow> cashflows;

    Date payment_date =
        issue_date + std::chrono::months{months_per_period};

    while (std::chrono::sys_days{payment_date} <
        ...
        payment_date =
            payment_date + std::chrono::months{months_per_period};
    }
